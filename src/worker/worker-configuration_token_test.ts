...
Deno.test("WorkerConfiguration - トークン制限設定", async (t) => {
  await t.step("デフォルト値を使用する場合", () => {
    ...
    const env = config.buildClaudeEnv();
    assertEquals(
      env.CLAUDE_CODE_MAX_OUTPUT_TOKENS,
      CLAUDE_CLI.DEFAULT_MAX_OUTPUT_TOKENS.toString(),
    );
  });
  ...
  await t.step("コンストラクタで指定した値を使用する場合", () => {
    ...
    const env = config.buildClaudeEnv();
    assertEquals(env.CLAUDE_CODE_MAX_OUTPUT_TOKENS, customTokens.toString());
  });

  await t.step("環境変数から値を取得する場合", () => {
    const envTokens = 30000;
    Deno.env.set("CLAUDE_CODE_MAX_OUTPUT_TOKENS", envTokens.toString());

    const config = new WorkerConfiguration();
    assertEquals(config.getMaxOutputTokens(), envTokens);

    const env = config.buildClaudeEnv();
    assertEquals(env.CLAUDE_CODE_MAX_OUTPUT_TOKENS, envTokens.toString());

    // テスト後のクリーンアップ
    Deno.env.delete("CLAUDE_CODE_MAX_OUTPUT_TOKENS");
  });

  await t.step("環境変数の無効な値を処理する場合", () => {
    // 無効な値を設定
    Deno.env.set("CLAUDE_CODE_MAX_OUTPUT_TOKENS", "invalid");
  ...
  await t.step("環境変数に負の値を設定した場合", () => {
    Deno.env.set("CLAUDE_CODE_MAX_OUTPUT_TOKENS", "-1000");
  ...
  await t.step("setMaxOutputTokensメソッドで値を変更する場合", () => {
    ...
    const env = config.buildClaudeEnv();
    assertEquals(env.CLAUDE_CODE_MAX_OUTPUT_TOKENS, newTokens.toString());
  });
  ...
  await t.step("buildClaudeEnvが正しい形式で環境変数を返す場合", () => {
    const config = new WorkerConfiguration();
    const env = config.buildClaudeEnv();

    assertExists(env.CLAUDE_CODE_MAX_OUTPUT_TOKENS);
    assertEquals(typeof env.CLAUDE_CODE_MAX_OUTPUT_TOKENS, "string");

    // 数値として解析できることを確認
    const parsed = parseInt(env.CLAUDE_CODE_MAX_OUTPUT_TOKENS, 10);
