...
export class MessageFormatter {
  ...
  /**
   * コードブロックで安全に表示できるようにトリプルバッククォートをエスケープ
   */
  private escapeTripleBackticks(text: string): string {
   ...
   * コードブロックで表示するためのヘルパー
   */
  private wrapInCodeBlock(command: string): string {
    const sanitized = this.escapeTripleBackticks(
      this.stripAnsiCodes(command).replace(/\r\n/g, "\n"),
    );
    return `\`\`\`sh\n${sanitized}\n\`\`\``;
  }
  ...
  formatToolUse(item: Anthropic.Messages.ToolUseBlock): string | null {
    ...
    if (item.name === "Bash") {
      ...
      const command = typeof input?.command === "string"
        ? input.command
        : undefined;
      const commandBlock = command && command.length > 0
        ? this.wrapInCodeBlock(command)
        : null;

      if (commandBlock) {
        if (description && description !== command) {
          return `${toolIcon} **${item.name}**:\n${description}\n${commandBlock}`;
        }
        return `${toolIcon} **${item.name}**:\n${commandBlock}`;
      }
    }
  ...
  formatToolResult(content: string, isError: boolean): string {
    const sanitized = this.stripAnsiCodes(content);
    if (!sanitized.trim()) {
      return "```\n(空の結果)\n```";
    }

    const normalized = sanitized.replace(/\r\n/g, "\n");
    const safeContent = this.escapeTripleBackticks(normalized);
